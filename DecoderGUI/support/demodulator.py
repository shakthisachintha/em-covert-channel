...
from support.hammingCode import Hamming
from support.decoder import Decoder

#region Config Parsing
config = configparser.ConfigParser()
config.read('./support/config.ini')
...
class Demodulator:
    ...
    def __init__(self, preambleThres: float = 0.05, bitThres: float = 0.03):
        self._sampleBinPreamble = np.fromfile(open("./support/binPreamble.bin"), dtype=np.float64)
        self._sampleBinOne = np.fromfile(open("./support/BinOne.bin"), dtype=np.float64)
        self._sampleBinZero = np.fromfile(open("./support/BinZero.bin"), dtype=np.float64)
        ...
        while blockStart < len(data):
            ...
            if enabled and len(samples) >= windowsPerBit:
                ...
                if bitsDecoded >= numOfBitsToDecode:
                    ...
                    decoded = []
                    statusChange("Detecting preamble")
        statusChange("Decoding completed")
